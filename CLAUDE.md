...
> Реальное состояние дерева на сейчас скромнее плана: есть `canvas/{tools,operations}`,
> `components/{Toolbar,Properties,Canvas,TopBar,StatusBar}`, `styles/`. Папки
> `effects/`, `Panels/`, `MenuBar/`, `ControlBar/`, `AIPanel/`, `state/`, `i18n/`,
> `plugins/` появятся по мере соответствующих фаз. `TopBar/` — временный бренд-бар,
> станет частью Menu Bar / Control Bar в фазе 1.1.
...
- **1.1 (Layout):** 🟡 Toolbar/Canvas/Properties/Status Bar/тёмная тема ✅ · ⬜ **Menu Bar**, **Control Bar** (ещё не созданы; сейчас только временный `TopBar` с брендом)
- **1.2 (Холст/навигация):** 🟡 зум/пан ✅, Rectangle ✅, Contextual Task Bar ✅ · ⬜ Rotate View
- **2.1 (Toolbar/выделение):** 🟡 1/2 колонки + flyout ✅, Select/Direct/Group ✅, Hand/Zoom ✅ · ⬜ drawer, **Magic Wand**, **Lasso**
...
- **5.1 (Type):** 🟡 Point, Area, on a Path ✅ · ⬜ Vertical (все), Touch Type
...
**Следующее по плану:** **фаза 5.1** — Type Tools (вертикальные варианты, Touch Type;
Point/Area/on Path уже есть). При желании можно сначала закрыть ранние пробелы:
1.1 Menu/Control Bar, 2.1 Magic Wand/Lasso, 2.2 трансформации мышью (поворот, Reference
Point, Alt-копия), 3.1 live shapes, 3.2 полная обводка.
