...
### Итерация 5 — Текст (продвинутый) ← ТЕКУЩАЯ (частично)
...
- [ ] Type on a Path — НЕ сделано (сложно: раскладка глифов вдоль кривой)
- [ ] Межбуквенный интервал (tracking) — НЕ сделано (PointText/canvas не умеет per-item letter-spacing; нужен рендер по символам)
...
- [x] Верхняя панель с подписанными «окошками» инструментов (TopBar), общий список с левым рейлом
...
- Иконки+список инструментов вынесены в `src/components/toolItems.jsx` (общий для Toolbar и TopBar).
- Лейаут App стал колоночным: `TopBar` сверху, ниже строка `.app-body` (рейл/холст/панель).
...
- [x] Линия
- [x] Pen Tool
- [~] Текст (point + area + выравнивание + leading; type-on-path и tracking — TODO)
