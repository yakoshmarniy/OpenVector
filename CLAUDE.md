...
- [x] `magicWandTool.js` — клик по объекту выделяет все с похожей заливкой (или обводкой, если заливки
      нет); допуск TOL=0.16 по сумме RGB-разниц. Shift = добавить. Использует общий `createSelection`.
- [x] `lassoTool.js` — freehand-петля; на отпускании выделяет объекты, чьи центры внутри петли или
      которых петля пересекает (`contains`/`intersects`). Shift = добавить.
...
- Magic Wand/Lasso держат своё выделение (свой `createSelection`) и репортят в App через
  `ctx.onSelectionChange`; перемещение не делают (как в Illustrator) — для move переключиться на Select
  (выделение при этом сбрасывается — общее ограничение, общий стор выделения будет позже).
